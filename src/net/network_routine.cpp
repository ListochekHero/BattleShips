...
auto NetworkEngine::init_engine(end_point_e socket_type, int root_socket)
  ...
  if (root_socket != 0) {
    ...
    root_socket_slot =
        connection_pool_.push_to_pool(ConnectionEntry{});
  }
  if (!root_socket_slot) {
...
auto NetworkEngine::send_message_to(const ConnectionView& recipient_view,
  ...
  auto& recipient_connection{
      connection_pool_.get_object(recipient_slot)};
...
void NetworkEngine::release_connection_by_view(ConnectionView view_to_release) {
  ...
  if (connection.socket_handler_.is_socket_alive()) {
    release_connection(
        connection_pool_.get_object(slot_to_release),
        slot_to_release);
  }
}
...
void NetworkEngine::process_events(const std::vector<size_t>& event_slots) {
  for (size_t slot : event_slots) {
    ...
    if (pending_connection.connection_type_ == end_point_e::LISTENER) {
      ...
      network_raw_tasks_.try_push(slot);
      available_task_tags_.try_push(task_tag_e::NETWORK);
    }
  }
}
...
auto NetworkEngine::process_connection_impl(size_t pool_slot)
  ...
  ConnectionEntry& pending_connection{
      connection_pool_.get_object(pool_slot)};
